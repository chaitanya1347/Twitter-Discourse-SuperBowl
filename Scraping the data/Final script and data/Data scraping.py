...
def gather_tweets(country, filename, start_year, years_back):
  header = ["tweet_id", "text", "author_id", "created_at", "place_id", "like_count", "quote_count", "reply_count", "retweet_count", "conversation_id", "in_reply_to_user_id"]
  with open(filename + '.csv', 'a', encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(header)
  query = generate_query(country)
  client = tweepy.Client("AAAAAAAAAAAAAAAAAAAAAFcMZAEAAAAALRP%2BfjSOiikXDHTI%2FG%2FJ6gPDfno%3DOOtIVKcPEXmTGdngCAaVy8qoZOlUbcqDSlp6m2LZJZTRHsZl09")
    ...
    for j in range(24, 31):
      ...
      for k in range(0, 24):
        tweets = client.search_all_tweets(
                                            query=query,
                                            start_time=dt.datetime(current_year, 1, j, k, 0, 0),
                                            end_time=dt.datetime(current_year, 1, j, k, 59, 59),
                                            max_results=400,
        ...
        with open(filename + '.csv', 'a', encoding="utf-8") as f:
            ...
            if tweets.data:
                for tweet in tweets.data:
                    place_id = tweet.data['geo']['place_id'] if 'geo' in tweet.data and 'place_id' in tweet.data['geo'] else "N/A"
                    in_reply_to_user_id = tweet.data['in_reply_to_user_id'] if 'in_reply_to_user_id' in tweet.data else "N/A"
                    data = [
                            tweet.data['id'],
                            tweet.data['text'],
...
def get_replies(filename, year):
  ...
  while i < len(conv_ids):
    ...
    tweets = client.search_all_tweets(
                                    query=query,
                                    start_time=dt.datetime(year, 1, 24, 0, 0, 0),
                                    end_time=dt.datetime(year, 1, 31, 23, 59, 59),
                                    max_results=250,
                                    expansions=['author_id', 'geo.place_id', 'in_reply_to_user_id'],
                                    place_fields=['country'],
                                    user_fields=['id', 'location', 'public_metrics'],
...
def tweets_to_LIWC(filename):
  with open(filename + '.csv', 'r', encoding="utf8") as infile, open("text_" + filename + '.txt', 'w', encoding="utf8") as outfile:
    reader = DictReader(infile)
    for row in reader:
      outfile.write(row['text'] + '\n')
if __name__=="__main__":
    gather_tweets("CA", "CA_2017", 2017, 1) # <<<<<<<--------- Change here
    time.sleep(2)
    get_replies("CA_2017", 2017)  # <<<<<<<--------- Change here
    time.sleep(2)
    tweets_to_LIWC("CA_2017")