...
def get_replies(filename, year):
    ...
    while i < len(conv_ids):
        ...
        tweets = client.search_all_tweets(
            query=query,
            start_time=dt.datetime(year, 2, 3, 0, 0, 0),
            end_time=dt.datetime(year, 2, 7, 23, 59, 59),
            max_results=200,
            expansions=['author_id', 'geo.place_id', 'in_reply_to_user_id'],
            place_fields=['country'],
            user_fields=['id', 'location', 'public_metrics'],
...
if __name__ == "__main__":
    gather_tweets("US", "US_2017", 2017, 1)  # <<<<<<<--------- Change here
    time.sleep(2)
    get_replies("US_2017", 2017)  # <<<<<<<--------- Change here
    time.sleep(2)
    tweets_to_LIWC("US_2017")